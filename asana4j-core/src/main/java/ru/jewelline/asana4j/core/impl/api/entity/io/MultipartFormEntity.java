...
public class MultipartFormEntity implements SerializableEntity {
    ...
    private final byte[] getOpening(String name) {
        StringBuilder opening = new StringBuilder();
        opening.append("--" + this.boundary).append(LINE_END);
        opening.append("Content-Disposition: form-data; name=\"file\"; filename=\"" + name + "\"").append(LINE_END);
        opening.append("Content-Type: " + URLConnection.guessContentTypeFromName(name)).append(LINE_END);
        opening.append(LINE_END);
        return opening.toString().getBytes(StringUtils.getCharset());
    }

    private final byte[] getEnding() {
