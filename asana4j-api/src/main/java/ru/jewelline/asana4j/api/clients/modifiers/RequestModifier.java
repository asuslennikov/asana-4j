 ...
 * Implementation of this interface can can add additional query parameters to request (for example for pagination),
...
public interface RequestModifier {
     ...
     * to other modifiers). Basically all implementation will call {@link ModifiersChain#next(ApiRequestBuilder)},
     * but if it required by logic of your modifier, it is possible to stop further processing. There is no guarantee
     * that next modifier will be called immediately after the {@link ModifiersChain#next(ApiRequestBuilder)} call.
     ...
     * @param chain ordered list which holds all registered request modifiers
     */
    <T> void modify(ApiRequestBuilder<T> requestBuilder, HttpMethod httpMethod, ModifiersChain chain);
}
